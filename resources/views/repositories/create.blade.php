<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Repositorios') }}
        </h2>
    </x-slot>

    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-4">
                <form action="{{ route('repositories.store') }}" method="POST" class="max-w-mg">
                    @csrf
                    <label for="" class="block font-medium text-sm text-gray-700">URL</label>
                    <input type="text" class="form-input w-full rounded-md shadow-sm" name="url">

                    <label for="" class="block font-medium text-sm text-gray-700">Description</label>
                    <textarea name="description" class="form-input w-full rounded-md shadow-sm" id="description" cols="30" rows="10"></textarea>
                    <input type="submit" class="bg-blue-500 text-white font-bold py-2 px-4 rounded-md shadow-sm">Guardar</input>
                </form>
            </div>
        </div>
    </div>
</x-app-layout>
